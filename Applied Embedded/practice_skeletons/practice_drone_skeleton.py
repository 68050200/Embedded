...
def handle_mode_button(keys):
    ...
    # TODO: debounce — โครงเดียวกับ prep_02 / prep_03 เป๊ะ:
    #     if keys.b and not prev_b:
    #         smooth_mode = not smooth_mode
    #         in_range_frames = 0
    #         thrust = 0.0
    #     prev_b = keys.b
    # อธิบายให้ตัวเองได้ด้วยว่าทำไมบรรทัดสุดท้ายต้องอยู่นอก if
    pass
...
def move_target():
    ...
    # TODO: integrate + reflect ที่ TARGET_TOP และ TARGET_BOTTOM
    #       โครงจาก prep_01 — ขอบบนให้ดู ขอบล่างทำเองแบบเดียวกัน:
    #     target_y += target_vy
    #     if target_y < TARGET_TOP:
    #         target_y = TARGET_TOP           # ดันกลับเข้าขอบก่อน
    #         target_vy = -target_vy          # แล้วค่อยกลับทิศ
    #       แล้วเลื่อนขีดเส้นประทุกขีดไปที่ระดับใหม่:
...
def decide_thrust(error):
    ...
    # TODO โหมด SMOOTH — โครงเดียวกับ prep_03 (เติมเนื้อเอง):
    #     if error > DEADZONE:
    #         ...เร่งขึ้น ยังไม่ in_range...
    #     elif error < -DEADZONE:
    #         ...เร่งลง ยังไม่ in_range...
    #     else:
    #         ...ไม่เร่ง ปล่อย friction หน่วงจนนิ่ง = in_range...
    #     thrust = max(-THRUST_MAX, min(THRUST_MAX, thrust))   # clamp ชั้น 1
    #
    # TODO โหมด BANG-BANG:
    #   - เร่งสุด/ดิ่งสุดอย่างเดียว ไม่มี deadzone ไม่มีความเฉื่อย
    #     (ทำเพื่อให้เห็นกับตาว่ามอเตอร์สลับทิศรัวแค่ไหน)
    return False
...
def act_fly(push):
    ...
    # TODO: alt += thrust  แล้ว  alt -= GRAVITY  แล้ว  alt += push
    #       clamp ชั้น 2: alt อยู่ในช่วง 0 ถึง MAX_ALT
    #       แล้ว move_to ตัวโดรนกับใบพัดทั้งสองด้วย alt_to_y()
    #       (ใบพัดอยู่สูงกว่าตัวเครื่อง 6 px · ซ้ายที่ x=360 · ขวาที่ x=412)
    #
    # ระวัง: GRAVITY ต้องลบออกจาก "ความสูง" ไม่ใช่ลบออกจาก "แรงมอเตอร์"
    #        ถ้าลบออกจากแรงมอเตอร์ มันจะกลายเป็นมอเตอร์อ่อนแรง
    #        ซึ่งคนละเรื่องกับน้ำหนักตัวโดรน
    pass
