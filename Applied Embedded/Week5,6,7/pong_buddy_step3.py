...
def predict_ball_y():
    ...
    period = 2 * BALL_BOUNCE_H #ระยะทางเด้งขึ้นลง ขอบเขตที่ลูกเคลื่อนที่ได้ ชนขอบบน y = 0
    
    m = direct_y % period  #ตำแหน่งที่คาดการณ์ ตัดระยะที่เด้งครบรอบ(ขึ้นลง)ทิ้ง หาระยะทางที่เหลือ (เพื่อนำไปเช็กว่าเป็นเด้งขึ้นหรือเด้งลง)
    
    if m <= BALL_BOUNCE_H: #ระยะทางที่เหลือยังไม่เกินขอบล่าง เด้งลง
        predicted_y = m
        
    else: #ถ้าระยะทางที่เหลือเกินขอบล่างไปแล้ว ให้กลับทิศ เด้งขึ้น
        predicted_y = period - m
